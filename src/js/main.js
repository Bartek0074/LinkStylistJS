...
const linkHandle = (e) => {
	...
	for (let i = 0; i < links.length; i++) {
		if (links[i].classList.contains('link-type--active')) {
			...
			linksVariationBox[i].firstElementChild.classList.add(
				'link-variation--active'
			);
			...
			stageLink.classList.add(
				linksVariationBox[i].firstElementChild.classList[1]
			);
			...
			if (linksVariationBox[i].firstElementChild.classList.length === 4) {
				stageLinkWrapper.classList.add(
					linksVariationBox[i].firstElementChild.classList[2]
				);
			}
		}
	}
...
// TEXT VALUE OPTION
const textHandle = () => (stageLink.innerHTML = textInput.value);
